...
function imageDisplayRequested(query=''){
 ...
 return /(?:展示|显示|配图|图片墙|图片对比|产品图|商品图|竞品图|品牌图|找图|搜索图片|搜图|show|display|gallery)\s*(?:相关|产品|商品|品牌|竞品|图片|image)?/i
  .test(
   String(query)
  );
}
 ...
 for(const item of pages){
  ...
  try{
   ...
   return{
    url:checked.sourceUrl,
    title:entity,
    caption:`${entity} 的公开参考图片，具体产品型号以来源页面为准。`,
...
async function addDisplayMediaFallback(page,enabled){
 ...
 const entities=[...new Set(page.sections.flatMap(section=>section.links.map(link=>link.label)))]
  .filter(Boolean)
  .slice(0,3);
 ...
 const section={
  type:'product_image',
  heading:'品牌与竞品图片',
  intro:'以下为经过访问校验的公开参考图片；产品版本与型号请以来源页面为准。',
  items:[],
  rows:[],
  resultLabel:'',
  factIds:[],
...
const generatePage=
 async (
  ...
  await verifyDisplayMedia(
   page,
   displayImages
  );

  return addDisplayMediaFallback(
   page,
   displayImages
  );
 };
