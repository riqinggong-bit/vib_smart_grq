...
此目录是独立副本，不会修改上一级原项目。首屏恢复为参考样式的深色单输入框布局，不展示额外的图片上传控件；支持纯文字和 Query 图片 URL。

当 Query 明确要求“展示图片、产品图、竞品图或图片对比”时，结果页才会启用图片组件。模型返回的图片 URL 会经过服务端真实下载校验；没有可靠图片时，再按品牌候选从 Wikimedia Commons 查找带来源的公开图片。无法访问或无法验证的图片不会显示。
