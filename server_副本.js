...
async function requestModel(input,instructions,schema,parser,ground=false){const endpoint=isGemini?`${geminiBase}/v1beta/models/${encodeURIComponent(model)}:generateContent`:'https://api.openai.com/v1/responses';const base=isGemini?{systemInstruction:{parts:[{text:instructions}]},contents:[{role:'user',parts:[{text:input}]}],generationConfig:{responseMimeType:'application/json',responseJsonSchema:schema}}:{model,instructions,input,text:{format:{type:'json_schema',name:'liquid_page',strict:true,schema}}};const payloads=isGemini&&ground&&geminiGrounding?[{...base,tools:[{googleSearch:{}}]},base]:[base];let r,o,lastDetail='';for(const payload of payloads){for(let attempt=0;attempt<3;attempt++){r=await fetch(endpoint,{method:'POST',signal:AbortSignal.timeout(90000),headers:isGemini?{'Content-Type':'application/json','x-goog-api-key':apiKey()}:{'Content-Type':'application/json',Authorization:`Bearer ${apiKey()}`},body:JSON.stringify(payload)});if(r.ok)break;if(![429,500,502,503,504].includes(r.status)||attempt===2)break;await new Promise(resolve=>setTimeout(resolve,800*2**attempt+Math.random()*300))}if(r.ok){o=await r.json();break}try{const data=await r.json();lastDetail=data.error?.message||''}catch{}if(!(isGemini&&payload.tools&&[400,404].includes(r.status)))break}if(!r?.ok){const m={400:'模型或请求配置不受支持。',401:'API Key 无效。',403:'当前项目没有访问权限。',404:'模型不存在或无法访问。',429:'API 配额不足或请求过于频繁。',503:'模型服务繁忙，请稍后重试。'};const error=Error(m[r.status]||`模型服务暂时不可用（${r.status}）。`);error.detail=lastDetail;console.error('Model API error:',r.status,lastDetail);throw error}if(isGemini){const c=o.candidates?.[0];if(c?.finishReason!=='STOP')throw Error('模型未完成生成，请重试。');const wrapped={output:[{type:'message',content:(c.content?.parts||[]).filter(p=>p.text&&!p.thought).map(p=>({type:'output_text',text:p.text}))}]};return attachEvidence(parser(wrapped),evidenceOfGemini(o))}return parser(o)}
...
function createServer(){return http.createServer((req,res)=>{
 ...
 if(req.method==='GET'&&route==='/api/health')return send(res,200,{ready:Boolean(apiKey()&&apiKey()!=='your_api_key_here'),model,provider:isGemini?'Gemini':'OpenAI',geminiBase:isGemini?geminiBase:undefined,geminiGrounding:isGemini?geminiGrounding:undefined});
