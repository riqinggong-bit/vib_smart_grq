...
function sourceType(
 ...
 if(
  /gov|edu|官方|旗舰店|tesla|apple|xiaomi|honeywell/
   .test(text)
 ){
  return'official';
 }
...
async function requestCloseAI(
 ...
 const systemPrompt=
  instructions+
  '\n\n你必须严格按照下面 JSON Schema 输出。只输出合法 JSON，不要 Markdown，不得缺少字段或增加额外字段。'+
  '\nJSON Schema：\n'+
  schemaText;
   ...
   [
    {
     ...
     content:
      '你是 JSON 修复器。只输出严格符合下面 JSON Schema 的合法 JSON。\n'+
      schemaText
    },
    {
     role:
...
const planPrompt=
 '理解用户真正要完成的任务，提取全部硬约束，并规划恰好七种呈现同一份最终答案的网页形态。'+
 '七种形态依次对应 hand、terminal、magazine、ice、minimal、app、neon；'+
 '它们只改变叙事顺序、视觉重点与交互方式，不得改变事实、计算、候选项或最终结论。'+
 'sharedFacts 是统一事实账本：用户内容标 user/provided，无法核实的外部信息标 fact/unverified，'+
 '假设标 assumption/unverified，计算标 calculation/provided；'+
 '只有有直接搜索证据的外部事实才允许标 verified。不得伪造来源、价格、库存、参数或商品详情 URL。'+
 '商品推荐/产品选型/品牌型号对比时，把 product_image 作为建议组件之一。'+
 '普通上传图片主要用于理解，不默认展示原图。严格限制文字，中文，不追问。';


const pagePrompt=
 '生成一份真正完成用户任务的统一内容结果，供七种界面共同渲染。'+
 '必须保留全部约束，先给关键判断，再展示可比较信息、风险和可执行步骤。'+
 '若涉及购买、品牌、产品选型或商品对比：至少列出3个具体候选，优先生成 type=product_image 的 section；'+
 'media 中 entity/title 写具体品牌+型号，caption 写简短推荐理由；'+
 '如果能确认官网页/品牌页/可信商品页则写 sourceUrl，无法确认图片直链时 url 返回空字符串，后端会继续寻找可信产品图。'+
 'links 为候选项给出 official/jd/taobao 安全搜索入口，不得编造详情 URL。'+
 '存在用户上传原图时必须直接观察，明显信息纳入结果，看不清的标“图片信息待核实”。'+
 '所有外部事实必须来自 sharedFacts；无法核实的参数/价格写待核实。图片只做视觉展示，不作为参数真实性证据。'+
 'calculator rows 第一行固定为[名称,初值,最小值,最大值,步长,单位]，items 第一项仅 sum/product；'+
 'comparison/table/barChart 用 rows；checklist/timeline/steps/cards 用 items；未使用字段返回空数组。用中文。';
 ...
 (
  ...
  requestModel(
   JSON.stringify({
    ...
    imageContext:
     images.length
      ? '用户上传了原始参考图片，请重新直接观察图片并把可见内容纳入结果。'
      : ''
   }),
...
function productIdentity(
 ...
 stripped=
  stripped.replace(
   /官方旗舰店|官方旗舰|旗舰店|官方|旗舰|家用|宠物|除甲醛|除菌|智能|新款|升级版/g,
   ''
  );
...
function itempropImageCandidates(
 html,
 pageUrl
){
...
function plausibleProductImageUrl(
 raw=''
){
 ...
 try{
  ...
  return !/(?:favicon|sprite|avatar|qrcode|qr-code|wechat|logo(?:[._/-]|$)|icon(?:[._/-]|$)|header(?:[._/-]|$)|footer(?:[._/-]|$)|banner(?:[._/-]|$))/i
   .test(
    text
   );
...
async function verifiedImageUrl(
 raw,
 referer=''
){
...
function isMarketplaceDetailUrl(
 raw=''
){
...
/* =========================================================
   MARKETPLACE PAGE DISCOVERY
========================================================= */

async function searchMarketplaceProductPages(
 entity
){
...
/* =========================================================
   MARKETPLACE SEARCH CARD IMAGE FALLBACK
========================================================= */

async function searchMarketplaceImageCards(
 entity
){
 ...
 while(
  ...
  out.push({
   ...
   source:
    `${marketplaceLabel(sourceUrl)}搜索结果`
  });
 }
...
async function resolveProductMediaItem(
 ...
 /*
  * 1. 已有 Grounding / sourceUrl。
  *
  * 普通产品页阈值 55。
  */
 for(
  const sourceUrl
  of mediaEvidenceUrls(
   item,
   plan
  )
 ){
  ...
  if(
   ...
   console.warn(
    `[product-image] reject evidence page(score=${inspected.score||0}): ${entity} <- ${sourceUrl}`
   );
  ...
  if(
   ...
   console.warn(
    `[product-image] matched evidence page but no usable image: ${entity} <- ${sourceUrl}`
   );
  ...
  console.log(
   `[product-image] verified evidence image(score=${inspected.score}, via=${inspected.imageSource||'page'}): ${entity}`
  );
 ...
 const marketplacePages=
  await searchMarketplaceProductPages(
   entity
  );
 ...
 for(
  ...
  if(
   ...
   console.warn(
    `[product-image] reject marketplace page(score=${inspected.score||0}): ${entity} <- ${result.url}`
   );
  ...
  if(
   ...
   console.warn(
    `[product-image] matched marketplace page but no usable image: ${entity} <- ${result.url}`
   );
  ...
  console.log(
   `[product-image] marketplace detail image(score=${inspected.score}, via=${inspected.imageSource||'page'}): ${entity} <- ${result.url}`
  );
  ...
  return{
   ...
   source:
    `${marketplaceLabel(result.url)}商品详情页`,
 ...
 for(
  ...
  if(
   ...
   console.warn(
    `[product-image] reject search page(score=${inspected.score||0}): ${entity} <- ${result.url}`
   );
  ...
  if(
   ...
   console.warn(
    `[product-image] matched search page but no usable image: ${entity} <- ${result.url}`
   );
  ...
  console.log(
   `[product-image] verified search image(score=${inspected.score}, via=${inspected.imageSource||'page'}): ${entity} <- ${result.url}`
  );
  ...
  *
  * 必须：
  * - 来源绑定真实 JD/TB/Tmall 详情页
  * - 标题/型号达到匹配分数
  * - 图片 URL 可访问
  */
 const cards=
  await searchMarketplaceImageCards(
   entity
  );
 ...
 for(
  ...
  console.log(
   `[product-image] marketplace card fallback(score=${card.score}): ${entity} <- ${card.sourceUrl}`
  );
 ...
 console.warn(
  `[product-image] no verified product image found: ${entity}`
 );
