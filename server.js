...
async function requestModel(input,instructions,schema,parser){const endpoint=isGemini?`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`:'https://api.openai.com/v1/responses';const payload=isGemini?{systemInstruction:{parts:[{text:instructions}]},contents:[{role:'user',parts:[{text:input}]}],generationConfig:{responseMimeType:'application/json',responseJsonSchema:schema}}:{model,instructions,input,text:{format:{type:'json_schema',name:'liquid_page',strict:true,schema}}};let r;for(let attempt=0;attempt<3;attempt++){r=await fetch(endpoint,{method:'POST',signal:AbortSignal.timeout(90000),headers:isGemini?{'Content-Type':'application/json','x-goog-api-key':apiKey()}:{'Content-Type':'application/json',Authorization:`Bearer ${apiKey()}`},body:JSON.stringify(payload)});if(r.ok)break;if(![429,500,502,503,504].includes(r.status)||attempt===2)break;await new Promise(resolve=>setTimeout(resolve,800*2**attempt+Math.random()*300))}if(!r.ok){let detail='';try{const data=await r.json();detail=data.error?.message||''}catch{}const m={400:'模型或请求配置不受支持。',401:'API Key 无效。',403:'当前项目没有访问权限。',404:'模型不存在或无法访问。',429:'API 配额不足或请求过于频繁。',503:'模型服务繁忙，请稍后重试。'};const error=Error(m[r.status]||`模型服务暂时不可用（${r.status}）。`);error.detail=detail;console.error('Model API error:',r.status,detail);throw error}const o=await r.json();if(isGemini){const c=o.candidates?.[0];if(c?.finishReason!=='STOP')throw Error('模型未完成生成，请重试。');return parser({output:[{type:'message',content:(c.content?.parts||[]).filter(p=>p.text&&!p.thought).map(p=>({type:'output_text',text:p.text}))}]})}return parser(o)}
const planPrompt='理解用户真正要完成的任务，提取全部硬约束，并规划恰好七种呈现同一份最终答案的网页形态。七种形态依次对应 hand、terminal、magazine、ice、minimal、app、neon；它们只改变叙事顺序、视觉重点与交互方式，不得改变事实、计算、候选项或最终结论。sharedFacts 是统一事实账本：用户内容标 user/provided，无法核实的外部信息标 fact/unverified，假设标 assumption/unverified，计算标 calculation/provided；不得伪造 verified 或来源。每个形态给出适合的阅读场景、重点、相同交付目标、3至6节统一内容大纲与有效组件。严格限制文字，中文，不追问。';
const pagePrompt='生成一份真正完成用户任务的统一内容结果，之后会被七种界面共同渲染。必须保留全部约束，先给关键判断，再展示思考路径、可比较的信息、可执行步骤与最终选择。所有外部事实必须来自 sharedFacts；无法核实的参数或价格明确写待核实，不得编造品牌、链接、来源或精确数据。内容要具体，避免空泛建议。calculator 用 rows 表示输入字段，第一行固定为[名称,初值,最小值,最大值,步长,单位]；items 第一项只用 sum 或 product。comparison/table/barChart 使用 rows 且第一行表头，barChart 第二列为数字。checklist/timeline/steps/cards 用 items。未使用字段返回空数组。用中文。';
