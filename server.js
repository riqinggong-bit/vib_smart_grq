...
const geminiInteractions=process.env.GEMINI_USE_INTERACTIONS!=='false';
...
function modelError(status,detail=''){const m={400:'模型或请求配置不受支持。',401:'API Key 无效。',403:'当前项目没有访问权限。',404:'模型不存在或无法访问。',429:'API 配额不足或请求过于频繁。',503:'模型服务繁忙，请稍后重试。'};const error=Error(m[status]||`模型服务暂时不可用（${status}）。`);error.detail=detail;error.apiStatus=status;console.error('Model API error:',status,detail);return error}
async function postModel(endpoint,payload,headers){let r,o,lastDetail='';for(let attempt=0;attempt<3;attempt++){r=await fetch(endpoint,{method:'POST',signal:AbortSignal.timeout(90000),headers,body:JSON.stringify(payload)});if(r.ok){o=await r.json();return{r,o,lastDetail:''}}try{const data=await r.json();lastDetail=data.error?.message||data.message||''}catch{}if(![429,500,502,503,504].includes(r.status)||attempt===2)break;await new Promise(resolve=>setTimeout(resolve,800*2**attempt+Math.random()*300))}return{r,o:null,lastDetail}}
function interactionText(o){return(o.steps||[]).filter(x=>x.type==='model_output').flatMap(x=>x.content||[]).filter(x=>['text','output_text'].includes(x.type)&&typeof x.text==='string').map(x=>x.text).join('')}
async function requestGeminiInteractions(input,instructions,schema,parser,ground=false){const endpoint=`${geminiBase}/v1beta/interactions`,base={model,input,system_instruction:instructions,response_format:{type:'text',mime_type:'application/json',schema},store:false};const payloads=ground&&geminiGrounding?[{...base,tools:[{type:'google_search'}]},base]:[base];let last;for(const payload of payloads){last=await postModel(endpoint,payload,{'Content-Type':'application/json','x-goog-api-key':apiKey()});if(last.r?.ok){const text=interactionText(last.o);if(!text)throw Error('模型返回为空，请重试。');const wrapped={output:[{type:'message',content:[{type:'output_text',text}]}]};return parser(wrapped)}if(!(payload.tools&&[400,404].includes(last.r?.status)))break}throw modelError(last.r?.status||502,last.lastDetail)}
async function requestGeminiLegacy(input,instructions,schema,parser,ground=false){const endpoint=`${geminiBase}/v1beta/models/${encodeURIComponent(model)}:generateContent`;const base={systemInstruction:{parts:[{text:instructions}]},contents:[{role:'user',parts:[{text:input}]}],generationConfig:{responseMimeType:'application/json',responseSchema:schema}};const payloads=ground&&geminiGrounding?[{...base,tools:[{googleSearch:{}}]},base,{...base,generationConfig:{responseMimeType:'application/json'}}]:[base,{...base,generationConfig:{responseMimeType:'application/json'}}];let last;for(const payload of payloads){last=await postModel(endpoint,payload,{'Content-Type':'application/json','x-goog-api-key':apiKey()});if(last.r?.ok){const o=last.o,c=o.candidates?.[0];if(c?.finishReason&&c.finishReason!=='STOP')throw Error('模型未完成生成，请重试。');const wrapped={output:[{type:'message',content:(c?.content?.parts||[]).filter(p=>p.text&&!p.thought).map(p=>({type:'output_text',text:p.text}))}]};return attachEvidence(parser(wrapped),evidenceOfGemini(o))}if(![400,404].includes(last.r?.status))break}throw modelError(last.r?.status||502,last.lastDetail)}
async function requestModel(input,instructions,schema,parser,ground=false){if(isGemini){if(geminiInteractions){try{return await requestGeminiInteractions(input,instructions,schema,parser,ground)}catch(error){if(![400,404].includes(error.apiStatus))throw error;console.warn('Interactions API unavailable, falling back to generateContent:',error.detail||error.message)}}return requestGeminiLegacy(input,instructions,schema,parser,ground)}const endpoint='https://api.openai.com/v1/responses',payload={model,instructions,input,text:{format:{type:'json_schema',name:'liquid_page',strict:true,schema}}},result=await postModel(endpoint,payload,{'Content-Type':'application/json',Authorization:`Bearer ${apiKey()}`});if(!result.r?.ok)throw modelError(result.r?.status||502,result.lastDetail);return parser(result.o)}
...
function createServer(){return http.createServer((req,res)=>{
 ...
 if(req.method==='GET'&&route==='/api/health')return send(res,200,{ready:Boolean(apiKey()&&apiKey()!=='your_api_key_here'),model,provider:isGemini?'Gemini':'OpenAI',geminiBase:isGemini?geminiBase:undefined,geminiGrounding:isGemini?geminiGrounding:undefined,geminiInteractions:isGemini?geminiInteractions:undefined});
