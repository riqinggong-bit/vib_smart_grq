...
function modelError(status,detail=''){
 ...
 const error=Error(m[status]||`模型服务暂时不可用（${status}）。`);
 error.detail=detail;
 error.apiStatus=status;
 console.error('Model API error:',status,detail);
 return error
}

async function postModel(endpoint,payload,headers){
 let r,o,lastDetail='';
 for(let attempt=0;attempt<3;attempt++){
  ...
  if(r.ok){
   o=await r.json();
   return {r,o,lastDetail:''}
  }

  try{
   const data=await r.json();
   lastDetail=data.error?.message||data.message||''
  }catch{}

  if(![429,500,502,503,504].includes(r.status)||attempt===2)break;

  await new Promise(resolve=>
   setTimeout(resolve,800*2**attempt+Math.random()*300)
  )
 }

 return {r,o:null,lastDetail}
}
...
async function requestCloseAI(input,instructions,parser){
 const endpoint=`${closeAIBase}/chat/completions`;
 ...
 // 部分中转模型不支持 response_format，400 时自动降级
 if(!result.r?.ok && result.r?.status===400){
 ...
 const content=result.o?.choices?.[0]?.message?.content;

 if(!content){
  throw Error('模型返回为空，请重试。')
 }

 return parser(wrapText(content))
}
...
async function requestGeminiOfficial(
 ...
 for(const payload of payloads){
  ...
  if(![400,404].includes(last.r?.status))break
 }
...
function createServer(){return http.createServer((req,res)=>{
 ...
 if(req.method==='GET'&&route==='/api/health')return send(res,200,{ready:Boolean(apiKey()&&apiKey()!=='your_api_key_here'),model,provider:isGemini?'Gemini':'OpenAI',geminiBase:isGemini?geminiBase:undefined,geminiGrounding:isGemini?geminiGrounding:undefined,isCloseAI:isGemini?isCloseAI:undefined,closeAIBase:isCloseAI?closeAIBase:undefined});
